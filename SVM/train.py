...
def train(epochs=HYPERPARAMS.epochs, random_state=HYPERPARAMS.random_state,
          kernel=HYPERPARAMS.kernel, decision_function=HYPERPARAMS.decision_function, gamma=HYPERPARAMS.gamma,
          train_model=True):
    ...
    if train_model:
        ...
        print("--")
        print("Training samples: 28976")
        print("Testing samples: 3256")
        print("--")
        ...
        print(
            "  - validation accuracy = {0:.1f}".format(validation_accuracy * 100))
        ...
        print(
            "  - evalution time = {0:.1f} sec".format(time.time() - start_time))
...
parser.add_argument("-t", "--train", default="no",
                    help="if 'yes', launch training from command line")
parser.add_argument("-e", "--evaluate", default="no",
                    help="if 'yes', launch evaluation on test dataset")
