...
from Graphical_Visualisation import Emotion_Analysis, Emotion_Analysis_Video
...
def UploadVideo():
    ...
    return render_template('ManualUploadVideo.html')
...
@app.route('/uploadvideo', methods=['POST'])
def uploadvideo():
    """ Loads Video from System, does Emotion Analysis & renders."""
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            vidObj = cv2.VideoCapture(f"static/{filename}")
            ...
            while success:
                ...
                cv2.imwrite("static/frames/frame%d.jpg" % count, image)

                count += 1
                if count > 8000:
                    break
            result = Emotion_Analysis_Video(video)
            return render_template('VisualVideo.html', res=result, num=random.randint(1,4))
